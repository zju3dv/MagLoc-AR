 ...
 * All rights reserved.
 * @Author: xuehua
 * @Date: 2021-10-29 10:55:19
 * @LastEditTime: 2021-10-29 10:55:19
 * @LastEditors: xuehua
 */
#ifndef STATE_ESTIMATION_PREDICTION_MODEL_MOTION_MODEL_2D_VELOCITY_DIFF_YAW_H_
#define STATE_ESTIMATION_PREDICTION_MODEL_MOTION_MODEL_2D_VELOCITY_DIFF_YAW_H_
...
// motion model with 2d-velocity observations plus Gaussian noise,
// and differential yaw observation plus Gaussian noise.
class MotionModel2DVelocityDiffYawState : public State {
 public:
  ...
  void EstimateFromSamples(std::vector<State*> sample_state_ptrs, std::vector<double> weights) {
    ...
    MotionModel2DVelocityDiffYawState* my_state_ptr;
    Eigen::Matrix<double, 3, 1> z_vector = {0.0, 0.0, 1.0};
    for (int i = 0; i < sample_state_ptrs.size(); i++) {
      my_state_ptr = reinterpret_cast<MotionModel2DVelocityDiffYawState*>(sample_state_ptrs.at(i));
      if (i == 0) {
  ...
  void Add(State* state_ptr) {
    MotionModel2DVelocityDiffYawState* my_state_ptr = reinterpret_cast<MotionModel2DVelocityDiffYawState*>(state_ptr);
...
class MotionModel2DVelocityDiffYawStateUncertainty : public StateUncertainty {
 public:
  ...
  MotionModel2DVelocityDiffYawStateUncertainty(void) {
  ...
  ~MotionModel2DVelocityDiffYawStateUncertainty() {}
...
class MotionModel2DVelocityDiffYawControlInput : public ControlInput {
 public:
  void GetAllNamedValues(std::vector<std::pair<std::string, double>>* named_values) const {
    named_values->clear();
    named_values->emplace(named_values->end(), util::kNameVx, this->means_(0));
    named_values->emplace(named_values->end(), util::kNameVy, this->means_(1));
    named_values->emplace(named_values->end(), util::kNameDyaw, this->means_(2));
  }
  ...
  MotionModel2DVelocityDiffYawControlInput(void) {
    this->means_ = Eigen::Matrix<double, 3, 1>::Zero(3, 1);
    this->covariances_ = Eigen::Matrix<double, 3, 3>::Zero(3, 3);
  }

  ~MotionModel2DVelocityDiffYawControlInput() {}
...
class MotionModel2DVelocityDiffYaw : public PredictionModel {
 public:
  ...
  MotionModel2DVelocityDiffYaw(void) {
    this->mvg_sampler_ = sampler::MultivariateGaussianSampler();
  }

  ~MotionModel2DVelocityDiffYaw() {}

 private:
  sampler::MultivariateGaussianSampler mvg_sampler_;
};

class MotionModel2DVelocityDiffYawStateSampler : public StateSampler {
 public:
  ...
  void Sample(State* state_sample) {
    MotionModel2DVelocityDiffYawState* my_state_sample = reinterpret_cast<MotionModel2DVelocityDiffYawState*>(state_sample);
  ...
  double CalculateStateProbabilityLog(State* state_sample) {
    ...
    std::cout << "MotionModel2DVelocityDiffYaw::CalculateStateProbabilityLog" << std::endl;
#endif
    MotionModel2DVelocityDiffYawState* my_state_sample = reinterpret_cast<MotionModel2DVelocityDiffYawState*>(state_sample);
  ...
  MotionModel2DVelocityDiffYawStateSampler(void) {}

  ~MotionModel2DVelocityDiffYawStateSampler() {}
...
#endif  // STATE_ESTIMATION_PREDICTION_MODEL_MOTION_MODEL_2D_VELOCITY_DIFF_YAW_H_
