 ...
 * All rights reserved.
 * @Author: xuehua
 * @Date: 2021-09-07 15:04:04
 * @LastEditTime: 2021-09-07 15:04:28
 * @LastEditors: xuehua
 */
#ifndef STATE_ESTIMATION_PREDICTION_MODEL_MOTION_MODEL_CVCR_YAW_H_
#define STATE_ESTIMATION_PREDICTION_MODEL_MOTION_MODEL_CVCR_YAW_H_
...
// for this model, there is only 1DoF for orientation.
class MotionModelCVCRYawState : public State {
 public:
  ...
  void GetAllNamedValues(std::vector<std::pair<std::string, double>>* named_values) const {
    ...
    named_values->emplace(named_values->end(), util::kNameOmegaZ, this->omega_z_);
  }
  ...
  void EstimateFromSamples(std::vector<State*> sample_state_ptrs, std::vector<double> weights) {
    ...
    double mean_omega_z = 0.0;
    MotionModelCVCRYawState* my_state_ptr;
    Eigen::Matrix<double, 3, 1> z_vector = {0.0, 0.0, 1.0};
    for (int i = 0; i < sample_state_ptrs.size(); i++) {
      my_state_ptr = reinterpret_cast<MotionModelCVCRYawState*>(sample_state_ptrs.at(i));
      if (i == 0) {
      ...
      mean_omega_z = mean_omega_z + my_state_ptr->omega_z() * weights.at(i);
  ...
  void omega_z(double omega_z) {
    this->omega_z_ = omega_z;
  }
  ...
  std::string ToKey(std::vector<double> discretization_resolutions) {
    ...
    double yaw_resolution = discretization_resolutions.at(2);
    double omega_z_resolution = discretization_resolutions.at(3);
    ...
    double temp_omega_z = UniqueZeroRound(this->omega_z_, omega_z_resolution);
    return temp_position.ToKey() +
           "_" + std::to_string(temp_v_x) +
           "_" + std::to_string(temp_v_y) +
           "_" + std::to_string(temp_yaw) +
           "_" + std::to_string(temp_omega_z);
  }

  void Add(State* state_ptr) {
    MotionModelCVCRYawState* my_state_ptr = reinterpret_cast<MotionModelCVCRYawState*>(state_ptr);
    ...
    this->position_ = position;
    this->v_ += my_state_ptr->v();
    this->omega_z_ += my_state_ptr->omega_z();
  }
  ...
  void Multiply_scalar(double scalar_value) {
    ...
    position.floor(this->position_.floor());
    this->position_ = position;
    this->v_ *= scalar_value;
    this->omega_z_ *= scalar_value;
  }
...
class MotionModelCVCRYawStateUncertainty : public StateUncertainty {
 public:
  void CalculateFromSamples(std::vector<State*> sample_state_ptrs, std::vector<double> weights) {
    ...
    Eigen::Matrix<double, Eigen::Dynamic, 1> omega_zs = Eigen::Matrix<double, Eigen::Dynamic, 1>::Zero(sample_state_ptrs.size(), 1);
    Eigen::Matrix<double, 3, 1> z_vector = {0.0, 0.0, 1.0};
    ...
    for (int i = 0; i < sample_state_ptrs.size(); i++) {
      ...
      omega_zs(i) = my_state_ptr->omega_z();
    }
    ...
    this->omega_z_variance_ = (omega_zs.array() - omega_zs.mean()).pow(2.0).matrix().transpose() * weights_vector;
  }
  ...
  double omega_z_variance(void) {
    return this->omega_z_variance_;
  }

  MotionModelCVCRYawStateUncertainty(void) {
    ...
    this->omega_z_variance_ = 0.0;
  }

  ~MotionModelCVCRYawStateUncertainty() {}
 ...
 private:
  ...
  double omega_z_variance_;
};

class MotionModelCVCRYawControlInput : public ControlInput {
 public:
  void GetAllNamedValues(std::vector<std::pair<std::string, double>>* named_values) const {
    named_values->clear();
    ...
    named_values->emplace(named_values->end(), util::kNameAlphaZ, this->means_(2));
  }

  Eigen::Matrix<double, 3, 1> means(void) const {
    return this->means_;
  }

  void means(Eigen::Matrix<double, 3, 1> means) {
    this->means_ = means;
  }

  Eigen::Matrix<double, 3, 3> covariances(void) const {
    return this->covariances_;
  }

  void covariances(Eigen::Matrix<double, 3, 3> covariances) {
    this->covariances_ = covariances;
  }

  MotionModelCVCRYawControlInput(void) {
    this->means_ = Eigen::Matrix<double, 3, 1>::Zero(3, 1);
    this->covariances_ = Eigen::Matrix<double, 3, 3>::Identity(3, 3);
  }

  ~MotionModelCVCRYawControlInput() {}

 private:
  Eigen::Matrix<double, 3, 1> means_;
  Eigen::Matrix<double, 3, 3> covariances_;
};

class MotionModelCVCRYaw : public PredictionModel {
 public:
...
class MotionModelCVCRYawStateSampler : public StateSampler {
 public:
  void Init(std::set<std::string> sample_space_position_keys,
            ...
            double omega_z_min, double omega_z_max,
    ...
    this->omega_z_sampler_.Init(omega_z_min, omega_z_max);
    this->position_griding_resolution_ = position_griding_resolution;
  }

  void Sample(State* state_sample) {
    MotionModelCVCRYawState* my_state_sample = reinterpret_cast<MotionModelCVCRYawState*>(state_sample);
    ...
    my_state_sample->omega_z(this->omega_z_sampler_.Sample());
  }
  ...
  void Seed(int random_seed) {
    ...
    this->omega_z_sampler_.Seed(random_seed);
  }

  double CalculateStateProbabilityLog(State* state_sample) {
#ifdef DEBUG_FOCUSING
    std::cout << "MotionModelCVCRYawStateSampler::CalculateStateProbabilityLog" << std::endl;
#endif
    MotionModelCVCRYawState* my_state_sample = reinterpret_cast<MotionModelCVCRYawState*>(state_sample);
    ...
    double omega_z = my_state_sample->omega_z();
    ...
    double omega_z_min = this->omega_z_sampler_.min_value();
    double omega_z_max = this->omega_z_sampler_.max_value();
    ...
    if ((omega_z >= omega_z_min) && (omega_z <= omega_z_max)) {
      log_prob += std::log(1.0 / (omega_z_max - omega_z_min));
    } else {
      log_prob += std::log(0.0);
    }
  ...
  MotionModelCVCRYawStateSampler(void) {}

  ~MotionModelCVCRYawStateSampler() {}
 ...
 private:
  ...
  sampler::UniformRangeSampler<std::uniform_real_distribution<double>, double> omega_z_sampler_;
...
#endif  // STATE_ESTIMATION_PREDICTION_MODEL_MOTION_MODEL_CVCR_YAW_H_
